...
public class NameTest {
	...
	@ParameterizedTest(name = "{index} - name:[{0}], exceptedNotThrownException:{1}")
	@CsvSource(value = {";false", "' ';true", "0;true", "100;true"
		...
		, delimiter = ';')
	void generate(String name, boolean exceptedNotThrownException) {
		//given

		//when
		if (exceptedNotThrownException) {
			//then - not throw any Exception
			assertThatCode(() -> Name.generate(name)).doesNotThrowAnyException();
			return;
		}
		//then - throw NumberFormatException
		assertThatThrownBy(() -> Name.generate(name))
