...
- [x] 질문의 삭제 상태 변경
  ...
  - [x] 질문 삭제 시 예외 발생 TC 작성
    - [x] 질문 삭제 요청자와 질문 작성자가 다른 경우 예외 발생 검증 TC 작성
    - [x] 질문 작성자와 답변 작성자가 다른 경우 예외 발생 검증 TC 작성
- [ ] 질문과 답변의 삭제 이력 추가
- [ ] 질문의 답변 목록 객체를 일급 객체로 변환
  - [ ] 질문 삭제 조건 만족시, 질문 삭제 처리와 질문에 대한 답변 목록의 각 삭제 상태 변환을 일괄처리하는 메서드 구현
- [ ] domain/repository를 구분하여 패키지 구조 정리
- [ ] 질문과 해당질문에 대한 답변을 포함하여 삭제하는 경우 삭제 이력 일급 컬렉션을 활용하여 삭제이력 목록 객체를 생성

Service
- [ ] 기존 비지니스 로직을 도메인 계층에 분리한 각 비지니스 로직 호출부로 변경 

- 예외 처리
  - [ ] 이미 삭제된 질문을 삭제 하는 경우
  - [ ] 질문 삭제 요청자와 질문 작성자가 다른 경우
  - [ ] 질문과 답변의 작성자가 다른 경우 예외
  
- 코너 케이스
  - [ ] 질문 답변 중, 삭제된 답변이 포함된 경우
